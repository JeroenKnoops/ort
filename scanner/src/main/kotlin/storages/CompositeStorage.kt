...
class CompositeStorage(
    ...
    private fun fetchReadResult(readFunc: ScanResultsStorage.() -> Result<List<ScanResult>>): Result<List<ScanResult>> {
        if (readers.isEmpty()) return emptyResult()
...
/**
 * Return a success result with an empty list of [ScanResult]s.
 */
private fun emptyResult(): Result<List<ScanResult>> = Success(emptyList())
